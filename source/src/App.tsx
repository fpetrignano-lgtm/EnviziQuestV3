...
export default function Home(){
  ...
  const toggleFw=(id:string,col:"inUso"|"diInteresse")=>setFrameworkChecks(prev=>({...prev,[id]:{...(prev as any)[id],[col]:!((prev as any)[id]?.[col]??false)}}) as any);
  ...
  const [approachStepsShown,setApproachStepsShown]=useState(false);
  const [approachDataShown,setApproachDataShown]=useState(false);
  ...
  // CMD19: ordina missioni 1-5 per somma R+C (Data Foundation sempre prima)
  const missionOrderByRC=[0,...[1,2,3,4,5].sort((a,b)=>{const sumA=(needsByMissionHubFocused.find(([mi])=>mi===a)?.[1]||[]).reduce((s,n)=>{const rel=Math.min(needRelevance[n.id]??5,10);const crit=needCriticality[n.id]??5;return s+rel+crit;},0);const sumB=(needsByMissionHubFocused.find(([mi])=>mi===b)?.[1]||[]).reduce((s,n)=>{const rel=Math.min(needRelevance[n.id]??5,10);const crit=needCriticality[n.id]??5;return s+rel+crit;},0);return sumB-sumA;})];
  ...
  const setScreen=(next:Screen)=>{if(next===screen)return;setScreenHistory(history=>[...history,screen]);
    // CMD36: guard sequenza obbligatoria — missions hub richiede M0 completata
    if(next==="missions"&&!missionOutcomes[0]){setSelectedMission(0);setScreenState("briefing");return;}
    setScreenState(next);
  };
  ...
  const resumeAsProfile=()=>{if(!profile)return;setScreen("chapterMap");};
  ...
  const brandClick=()=>{if(profile)setScreen("chapterMap");else{setScreenState("onboarding");setScreenHistory([]);setProfile(null);setTrustScore(30);localStorage.removeItem("envizi-quest-trust-score");}};
  ...
  useEffect(()=>{if(questName.trim()){saveQuest(questName.trim());const now=new Date();setLastSaveTime(`${String(now.getHours()).padStart(2,"0")}:${String(now.getMinutes()).padStart(2,"0")}`);}},[screen]);
  // CMD10: marca approachStepsCopy come già mostrata
  useEffect(()=>{if(screen==="approachStepsCopy")setApproachStepsShown(true);},[screen]);
  // CMD12: marca approachDataCopy come già mostrata
  useEffect(()=>{if(screen==="approachDataCopy")setApproachDataShown(true);},[screen]);
  ...
  useEffect(()=>{
    ...
    root.render(
      <div className="globalSaveOverlay">
        {questName.trim()&&lastSaveTime&&!saveBtnOpen&&<span className="globalAutoSaveLabel">Salvato automaticamente · {lastSaveTime}</span>}
        {!saveBtnOpen
    ...
    return ()=>{};
  },[profile,saveBtnOpen,saveBtnName,questName,lastSaveTime]);
  ...
  const trustGainByOutcome=(outcome:Outcome,missionIndex?:number,ratings?:("alto"|"medio"|"basso")[])=>{const ratingVal={"alto":25,"medio":12,"basso":0};const gapScore=ratings&&ratings.length>0?ratings.reduce((s,r)=>s+ratingVal[r],0)/ratings.length:0;const gapBonus=Math.round((gapScore/25)*3);if(missionIndex===0&&outcome==="positive")return 25+gapBonus;return outcome==="positive"?15+gapBonus:outcome==="warning"?7+Math.round(gapBonus/2):0;};
  const calculatedTrustScore=Object.entries(missionOutcomes).reduce((total,[mi,o])=>total+trustGainByOutcome(o as Outcome,Number(mi),asIsRatings[Number(mi)]),30);
  ...
  const trustSteps=([
    ...
    ...missionOrder.map((mi,pos)=>{
      ...
      let cum=30;for(let p=0;p<=pos;p++){const o=missionOutcomes[missionOrder[p]];if(o)cum=Math.min(100,cum+trustGainByOutcome(o,missionOrder[p],asIsRatings[missionOrder[p]]));}
  ...
  if(screen==="chapterMap")return <ChapterMap language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} name={name} missionOrder={missionOrder} missionOutcomes={missionOutcomes} trustScore={trustScore}/>;

  // ── SLIDE DI TRANSIZIONE SEZIONI ──────────────────────────────────────────
  if(screen==="sectionIntro1"&&profile)return <SectionIntroSlide language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} num={1} labelIt="Introduzione" labelEn="Introduction" titleIt="Il percorso Envizi Quest" titleEn="The Envizi Quest journey" subIt="Come funziona, cosa scoprirai e come costruire la tua roadmap ESG" subEn="How it works, what you'll discover and how to build your ESG roadmap" nextScreen="questIntro" frozen/>;

  if(screen==="sectionIntro2"&&profile)return <SectionIntroSlide language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} num={2} labelIt="Obiettivi e dati" labelEn="Objectives and data" titleIt="Partiamo dagli obiettivi della tua azienda" titleEn="Let's start from your company's objectives" subIt="Priorità di business ESG e profilo aziendale" subEn="ESG business priorities and company profile" nextScreen="companySetup" frozen/>;

  if(screen==="sectionIntro3"&&profile)return <SectionIntroSlide language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} num={3} labelIt="Sfide di dati" labelEn="Data challenges" titleIt="Il secondo filone è agire sulle sfide di dati" titleEn="The second track is acting on data challenges" subIt="Esigenze, criticità e matrice di priorità" subEn="Needs, criticalities and priority matrix" nextScreen="approachDataCopy"/>;

  if(screen==="sectionOutro"&&profile)return <SectionIntroSlide language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} num={"✓" as any} labelIt="Prossimi passi" labelEn="Next steps" titleIt="Porta i tuoi risultati al livello successivo" titleEn="Take your results to the next level" subIt="Demo, Proof of Concept e Business Value Assessment" subEn="Demo, Proof of Concept and Business Value Assessment" nextScreen="nextStep"/>;

  if(screen==="questIntro"&&profile)return <QuestIntro language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} name={name}/>;


  if(screen==="blank1")return <Blank1 language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} p10SlideIdx={p10SlideIdx} setP10SlideIdx={setP10SlideIdx} P10_SLIDES={P10_SLIDES}/>;

  if(screen==="p10Slideshow"&&profile)return <P10Slideshow language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} p10SlideIdx={p10SlideIdx} setP10SlideIdx={setP10SlideIdx} P10_SLIDES={P10_SLIDES}/>;


  if(screen==="approachIntro"&&profile)return <ApproachIntro language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} t={t}/>;
  if(screen==="approachReport"&&profile)return <ApproachReport language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} t={t}/>;
  ...
  if(screen==="reportSlideshow"&&profile){
    ...
    return <ReportSlideshow language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} data={rd}/>;
  }

  if(screen==="reportSlideshowPng"&&profile)return <P10Slideshow language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} p10SlideIdx={reportSlideIdx} setP10SlideIdx={setReportSlideIdx} P10_SLIDES={REPORT_SLIDES_BUSTED} backScreen="ilTuoReport"/>;
  ...
  if(screen==="ilTuoReport"&&profile)return <IlTuoReport language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} p10SlideIdx={p10SlideIdx} setP10SlideIdx={setP10SlideIdx} P10_SLIDES={P10_SLIDES} onDownloadPptx={(lang)=>generateTemplatePptx(buildPptxData(lang==="it"))} onRefreshAndView={refreshAndViewReport}/>;
        ...
        <div className="esgStrContent">
          {matrixDone?(
            <>
              ...
              <p style={{margin:"8px 0 0",fontSize:"clamp(11px,1vw,13px)",color:"#5a8a78",lineHeight:1.5,fontStyle:"italic"}}>{isIt?"I punti fiducia riflettono la qualità e la profondità dell'analisi svolta, non la scelta di una tecnologia specifica.":"The trust score reflects the quality and depth of the analysis carried out, not the choice of any specific technology."}</p>
  ...
  if(screen==="challengeSeparator1"&&profile)return <ChallengeSeparator1 language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} missionIndex={missionOrder[0]}/>;

  if(screen==="missionCard1"&&profile)return <MissionCardScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} missionIndex={0} cardNum={1} backScreen="challengeSeparator1" needs={[...( needsByMissionHubFocused.find(([mi])=>mi===0)?.[1]||[]), {id:"__foundation__",label:language==="it"?"Baseline proposta — da validare":"Proposed baseline — to be validated"}]} priorities={priorities} needRelevance={needRelevance} needCriticality={needCriticality} needIdToCapability={needIdToCapability} setSelectedMission={setSelectedMission}/>;
  ...
  if(screen==="missionIntro"&&profile){const mid=t.missionIntroData[selectedMission]||t.missionIntroData[0];const mBody=(mid.body as string).replace("COMPANY_NAME",displayCompanyName);const activeMission=missionCatalog[selectedMission];return <main className="mission0IntroScreen"><header className="missionNav missionNavTrust"><button className="brand brandButton" onClick={reset}><span className="brandMark">e·</span><span>Envizi<br/>Impact Quest</span></button><div className="missionProgress"><span className="activeDot"/> {mid.eyebrow}</div>{renderTrustBar()}<div className="introNavRight"><button className="introBackBtn" onClick={()=>goBack()}>← {language==="it"?"Indietro":"Back"}</button><button className="langMini" onClick={()=>setLanguage(language==="it"?"en":"it")}>{language==="it"?"EN":"IT"}</button></div></header><section className="m0iStage"><img src={`./characters/${profile}-neutral.png`} alt={name} className="m0iProfileImg"/><div className="m0iPersonaTag"><span className="statusDot"/><div><small>ESG MANAGER</small><strong>{name}</strong></div></div></section><section className="m0iContent"><div className="m0iMissionBadge"><span>{activeMission.icon}</span><i>{mid.eyebrow}</i></div><h1 className="m0iTitle">{mid.title}</h1><p className="m0iKicker">{mid.kicker}</p><p className="m0iQuestion">{mid.question}</p><p className="m0iBody">{mBody}</p><button className="actionButton m0iCta" onClick={()=>setScreen("asis")}>{mid.cta}<b>→</b></button></section></main>;}



  if(screen==="summary"&&profile)return <SummaryScreen language={language} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} priorities={priorities} priorityIncluded={priorityIncluded} missionOrder={missionOrder} missionOutcomes={missionOutcomes} needsByMissionHub={needsByMissionHub} calculatedTrustScore={calculatedTrustScore} decisionLabel={decisionLabel} outcomeLabel={outcomeLabel} t={t}/>;

  if(screen==="nextStep"&&profile)return <NextStepScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} priorities={priorities} missionOrder={missionOrder} missionOutcomes={missionOutcomes} missionParameters={missionParameters} trustScore={trustScore} contactEmail={contactEmail} setContactEmail={setContactEmail} approachBiz={approachBiz} approachData={approachData} decisionLabel={decisionLabel} missionItems={missionItems} missionUnits={missionUnits} renderSaveBtn={renderSaveBtn} t={t} name={name}/>;


  if(screen==="milestone"&&profile)return <MilestoneScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} missionOutcomes={missionOutcomes} renderSaveBtn={renderSaveBtn} name={name}/>;


  if(screen==="thankYou"&&profile)return <ThankYouScreen language={language} setLanguage={setLanguage} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} t={t}/>;



  // ── ENERGIA E DECARBONIZZAZIONE — Foundation + Conclusion ──────────────────
  if(screen==="challengeComplete1"&&profile)return <ChallengeComplete1 language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar}/>;
  // ── SEPARATORI E COMPLETAMENTI SFIDE 2-6 ──────────────────────────────────
  if(screen==="challengeSeparator2"&&profile)return <ChallengeSeparator2 language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} missionIndex={missionOrder[1]}/>;
  if(screen==="missionCard2"&&profile)return <MissionCardScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} missionIndex={missionOrder[1]} cardNum={2} backScreen="challengeSeparator2" needs={needsByMissionHubFocused.find(([x])=>x===missionOrder[1])?.[1]||[]} priorities={priorities} needRelevance={needRelevance} needCriticality={needCriticality} needIdToCapability={needIdToCapability} setSelectedMission={setSelectedMission}/>;
  if(screen==="challengeComplete2"&&profile)return <ChallengeComplete2 language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar}/>;
  if(screen==="challengeSeparator3"&&profile)return <ChallengeSeparator3 language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} missionIndex={missionOrder[2]}/>;
  if(screen==="missionCard3"&&profile)return <MissionCardScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} missionIndex={missionOrder[2]} cardNum={3} backScreen="challengeSeparator3" needs={needsByMissionHubFocused.find(([x])=>x===missionOrder[2])?.[1]||[]} priorities={priorities} needRelevance={needRelevance} needCriticality={needCriticality} needIdToCapability={needIdToCapability} setSelectedMission={setSelectedMission}/>;
  if(screen==="challengeComplete3"&&profile)return <ChallengeComplete3 language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar}/>;
  if(screen==="challengeSeparator4"&&profile)return <ChallengeSeparator4 language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} missionIndex={missionOrder[3]}/>;
  if(screen==="missionCard4"&&profile)return <MissionCardScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} missionIndex={missionOrder[3]} cardNum={4} backScreen="challengeSeparator4" needs={needsByMissionHubFocused.find(([x])=>x===missionOrder[3])?.[1]||[]} priorities={priorities} needRelevance={needRelevance} needCriticality={needCriticality} needIdToCapability={needIdToCapability} setSelectedMission={setSelectedMission}/>;
  if(screen==="challengeComplete4"&&profile)return <ChallengeComplete4 language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar}/>;
  if(screen==="challengeSeparator5"&&profile)return <ChallengeSeparator5 language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} missionIndex={missionOrder[4]}/>;
  if(screen==="missionCard5"&&profile)return <MissionCardScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} missionIndex={missionOrder[4]} cardNum={5} backScreen="challengeSeparator5" needs={needsByMissionHubFocused.find(([x])=>x===missionOrder[4])?.[1]||[]} priorities={priorities} needRelevance={needRelevance} needCriticality={needCriticality} needIdToCapability={needIdToCapability} setSelectedMission={setSelectedMission}/>;
  if(screen==="challengeComplete5"&&profile)return <ChallengeComplete5 language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar}/>;
  if(screen==="challengeSeparator6"&&profile)return <ChallengeSeparator6 language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} missionIndex={missionOrder[5]}/>;
  if(screen==="missionCard6"&&profile)return <MissionCardScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} missionIndex={missionOrder[5]} cardNum={6} backScreen="challengeSeparator6" needs={needsByMissionHubFocused.find(([x])=>x===missionOrder[5])?.[1]||[]} priorities={priorities} needRelevance={needRelevance} needCriticality={needCriticality} needIdToCapability={needIdToCapability} setSelectedMission={setSelectedMission}/>;
  if(screen==="challengeComplete6"&&profile)return <ChallengeComplete6 language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar}/>;

  if(screen==="energyFoundation"&&profile)return <EnergyFoundationScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} efRatings={efRatings} setEfRating={setEfRating}/>;

  if(screen==="energyConclusion"&&profile)return <EnergyConclusionScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} efRatings={efRatings} missionOutcomes={missionOutcomes}/>;

  // ── COINVOLGIMENTO SUPPLY CHAIN — Foundation + Conclusion ───────────────────
  if(screen==="supplyFoundation"&&profile)return <SupplyFoundationScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} scRatings={scRatings} setScRating={setScRating}/>;

  if(screen==="supplyConclusion"&&profile)return <SupplyConclusionScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} scRatings={scRatings} missionOutcomes={missionOutcomes}/>;

  // ── ROTTA VERSO NET ZERO — Foundation + Conclusion ──────────────────────────
  if(screen==="planningFoundation"&&profile)return <PlanningFoundationScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} plRatings={plRatings} setPlRating={setPlRating}/>;

  if(screen==="planningConclusion"&&profile)return <PlanningConclusionScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} plRatings={plRatings} missionOutcomes={missionOutcomes}/>;

  // ── FRAMEWORK ESG E DISCLOSURE — Foundation + Conclusion ───────────────────
  if(screen==="frameworkFoundation"&&profile)return <FrameworkFoundationScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} frRatings={frRatings} setFrRating={setFrRating}/>;

  if(screen==="frameworkConclusion"&&profile)return <FrameworkConclusionScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} frRatings={frRatings} missionOutcomes={missionOutcomes}/>;


  if(screen==="reportingFoundation"&&profile)return <ReportingFoundationScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} rfRatings={rfRatings} setRfRating={setRfRating}/>;

  if(screen==="reportingConclusion"&&profile)return <ReportingConclusionScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} rfRatings={rfRatings} missionOutcomes={missionOutcomes}/>;


  if(screen==="dataFoundation"&&profile)return <DataFoundationScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} dfRatings={dfRatings} setDfRating={setDfRating}/>;


  if(screen==="dfConclusion"&&profile)return <DFConclusionScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} renderTrustBar={renderTrustBar} dfRatings={dfRatings} missionOutcomes={missionOutcomes} t={t}/>;
  ...
  if((screen==="briefing"||screen==="asis"||screen==="trust"||screen==="negative"||screen==="success")&&profile)return <MissionFlowScreen language={language} profile={profile} setLanguage={setLanguage} setScreen={setScreen} reset={brandClick} goBack={goBack} screen={screen} selectedMission={selectedMission} missionOutcomes={missionOutcomes} active={active} asIsRatings={asIsRatings} setAsIsRatings={setAsIsRatings} negativeChoice={negativeChoice} pendingOutcome={pendingOutcome} missionParameters={missionParameters} companyDims={companyDims} displayCompanyName={displayCompanyName} activeTrustIntro={activeTrustIntro} activeTrustSources={activeTrustSources} trustSteps={trustSteps} trustTotalW={trustTotalW} TRUST_BAR_W={TRUST_BAR_W} TRUST_BAR_GAP={TRUST_BAR_GAP} TRUST_CHART_H={TRUST_CHART_H} TRUST_SVG_PAD_X={TRUST_SVG_PAD_X} resultValues={resultValues} setPmMissionFilter={setPmMissionFilter} setPmFromBriefing={setPmFromBriefing} renderSaveBtn={renderSaveBtn} missionItems={missionItems} missionUnits={missionUnits} t={{...t,companyFacts:dynamicCompanyFacts}} name={name}/>;
  ...
  if(screen==="welcome"){
    ...
    return <main className="welcomeScreen" style={{position:"relative"}}>
      ...
      <span style={{position:"fixed",bottom:"20px",right:"24px",fontSize:"22px",fontWeight:700,letterSpacing:".18em",color:"rgba(57,239,180,.75)",fontFamily:"var(--font-geist-mono,monospace)",pointerEvents:"none",zIndex:9999}}>V3</span>
